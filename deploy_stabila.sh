...
if [ ! -e $BIN_PATH ]; then
  mkdir -p $BIN_PATH
  cd $BIN_PATH
  git clone https://github.com/stabilaprotocol/$PROJECT
fi

cd $BIN_PATH
if [ $NET == "mainnet" ]; then
  wget https://raw.githubusercontent.com/stabilaprotocol/stabila-deployment/master/main_net_config.conf -O main_net_config.conf
  ...
  wget https://raw.githubusercontent.com/stabilaprotocol/stabila-deployment/master/test_net_config.conf -O test_net_config.conf
...
while [ $count -le 360 ]; do
  ...
  if [ -n "$pid" ]; then
    kill -15 $pid
    echo "kill -15 java-stabila, counter $count"
    sleep 5
  else
    echo "$APP killed"
    break
